...
with tf.device('/device:GPU:0'):
    ...
    def cluster_groups_into_events(params, dataset, remove_singles=True, prefix=None, optimise_k=False,
        ...
        if optimise_k:
            ...
            best_std = 0
            best_k = 0
            for k in range(2, 31):  # could do (6, 21) -> min. 6 Configs through inspection + score worsens beyond 20
    ...
    def events_summary(params, dataset, prefix=None, save_figs=False, avg_group=False):
        ...
        with open(event_path, 'rb') as loader:
            ...
            for e in k_events.top_events[:, 0]:
                ...
                if params['molecule'] == 'BPT':
                    if avg_group:
                        new_path = f'./analysis/{params["c_ver"]}/events/{params["name"]}/{prfx}{k}clusters_avg/config{event.events[e].id}'
                    else:
                        new_path = f'./analysis/{params["c_ver"]}/events/{params["name"]}/{prfx}{k}clusters/config{event.events[e].id}'
                else:
                    if avg_group:
